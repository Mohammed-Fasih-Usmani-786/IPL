...
if (isset($_POST['start'])) //update data
{
    $status = $_POST["status"];


    $qry = "update auction set status='$status' where id=1";
    mysqli_query($connection, $qry) or die("not fire");
    if ($status == "True") {
        header("location:auction.php?m=Auction Start'");
    } else {
        header("location:auction.php?m=Auction Closed");
    }
}
